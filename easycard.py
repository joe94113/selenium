...
class Card:
    ...
    def enter_card_number(self):
        card_number_input = self.driver.find_element_by_xpath(
            '/html/body/form/div/div[1]/div[2]/div[2]/div/ul/li[1]/input')
        card_number_input.send_keys(CARD_NUMBER)

    def enter_birthday(self):
        birthday_input = self.driver.find_element_by_xpath(
            '/html/body/form/div/div[1]/div[2]/div[2]/div/ul/li[2]/input')
        birthday_input.send_keys(BIRTHDAY)

    def select_search_range(self): # 使用者按enter才繼續
        button = self.driver.find_element_by_id('date3m')
        button.send_keys(Keys.SPACES)
